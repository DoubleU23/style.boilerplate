...
function plugin() {
  return function(style){
    ...
    // tbd - define env-vars to inject (+ varPrefix '$ENV__') on init
    style.define('$ENV__NODE_ENV', process.env.NODE_ENV);
    style.define('$ENV__CLIENT', process.env.CLIENT);
    style.define('$ENV__ROOT_PATH', process.env.ROOT_PATH);

    style.define('assetPath', function (input) {
        imgUrlPrefix  = imgUrlPrefix || process.env.ROOT_PATH + '/assets';
